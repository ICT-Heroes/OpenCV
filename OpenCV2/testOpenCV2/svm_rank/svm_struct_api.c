...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  if(0) {
   /* we have only one big example */
    ...
    read_documents(file,&examples[0].x.doc,&examples[0].y.classs,&totwords,&n);
	...
	y->loss=0;
      ...
      y->classs=(double *)realloc(y->classs,sizeof(double)*y->totdoc);
      y->classs[y->totdoc-1]=labels[i];
    }
    sample.examples=examples;
    free(instances);
    free(labels);
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  y.totdoc=x.totdoc;
  y.classs=(double *)my_malloc(sizeof(double)*y.totdoc);
  y.factor=NULL;
  y.loss=-1;
  /* simply classify by sign of inner product between example vector
     and weight vector */
  for(i=0;i<x.totdoc;i++) {
    y.classs[i]=classify_example(sm->svm_model,x.doc[i]);
  }
  return(y);
}
						     ...
						     STRUCT_LEARN_PARM *sparm)
{
  ...
  ybar.classs=(double *)my_malloc(sizeof(double)*x.totdoc);
  score=ybar.classs;
...
double swappedpairs(LABEL y, LABEL ybar)
{
  ...
  for(i=0;i<y.totdoc;i++) {
    for(j=0;j<y.totdoc;j++) {
      if((y.classs[i]>y.classs[j]) && (ybar.classs[i]<=ybar.classs[j])) {
	sum++;
      }
    }
  }
...
double fracswappedpairs(LABEL y, LABEL ybar)
{
  ...
  for(i=0;i<y.totdoc;i++) {
    for(j=0;j<y.totdoc;j++) {
      if(y.classs[i]>y.classs[j]) {
	totpairs++;
	if(ybar.classs[i]<=ybar.classs[j]) {
	  sum++;
	}
      }
    }
