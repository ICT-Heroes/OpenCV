			   ...
			   int *alg_type)
{
  ...
  strcpy_s (modelfile, 17, "svm_struct_model");
  strcpy_s(learn_parm->predfile, 18,"trans_predictions");
  strcpy_s(learn_parm->alphafile, 1, "");
  ...
  strcpy_s(kernel_parm->custom, 6,"empty");
  strcpy_s(type, 2,"c");
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      ...
      //case 'a': i++; strcpy_s(learn_parm->alphafile, ,argv[i]); break;
  ...
  strcpy_s(trainfile, strlen(argv[i]) + 1, argv[i]);
  if((i+1)<argc) {
    strcpy_s(modelfile, strlen(argv[i+1]) + 1, argv[i+1]);
  }
...
void print_help()
{
  ...
  printf("    Kernel Methods - Support Vector Learning, B. Sch?kopf and C. Burges and\n");
