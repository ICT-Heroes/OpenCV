			   ...
			   long *verbosity,long *struct_verbosity)
{
  ...
  strcpy_s (predictionsfile, 16, "svm_predictions"); 
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      ...
      case '-': strcpy_s(struct_parm->custom_argv[struct_parm->custom_argc++], strlen(argv[i]) + 1,argv[i]);i++; strcpy_s(struct_parm->custom_argv[struct_parm->custom_argc++], strlen(argv[i])+1,argv[i]);break; 
  ...
  strcpy_s (testfile, strlen(argv[i]) + 1, argv[i]);
  strcpy_s(modelfile, strlen(argv[i+1]) + 1, argv[i + 1]);
  if((i+2)<argc) {
	  strcpy_s(predictionsfile, strlen(argv[i+2]) + 1, argv[i + 2]);
  }
